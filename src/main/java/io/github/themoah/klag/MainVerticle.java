...
public class MainVerticle extends AbstractVerticle {
  ...
  private final List<KafkaClientService> healthKafkaClients = new ArrayList<>();
  ...
  public void start(Promise<Void> startPromise) {
    ...
    for (KafkaClusterSpec spec : clusters) {
      ...
      KafkaClientService healthClient = new KafkaClientServiceImpl(vertx, spec.clientConfig());
      healthKafkaClients.add(healthClient);
      healthMonitors.add(new KafkaHealthMonitor(
        vertx, healthClient, appConfig.healthCheckIntervalMs(),
    ...
    // /readyz stays DOWN until a KafkaHealthMonitor reports UP.
    startHttpServer(router, appConfig.httpPort())
      .compose(server -> {
        httpServer = server;
        return joinAll(healthMonitors.stream()
  ...
  public void stop(Promise<Void> stopPromise) {
    ...
    joinAll(healthMonitors.stream().map(KafkaHealthMonitor::stop).toList())
      ...
      .compose(v -> joinAll(healthKafkaClients.stream().map(KafkaClientService::close).toList()))
      .onSuccess(v -> {
